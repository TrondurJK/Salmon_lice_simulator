...
class Planktonic_agent:
    ...
    def __init__(self, delta_time, save_time=40):
        ...
        self.smittu_count = np.zeros((cols), dtype=float)
        ...
        for i in range(cols):
            ...
            while start_iter < min(end, save_time):
                ...
                if c < end:
                    self.A[int(np.floor(start_iter)), i] = (c - start_iter) / (
                        end - start
                    )
                    start_iter = c
                else:
                    self.A[int(np.floor(start_iter)), i] = (end - start_iter) / (
                        end - start
                    )
                    start_iter = end
    ...
    def update(self, gravid_lice, temp):
        ...
        self.smittu_count = np.hstack(
            (
                [gravid_lice * self.interp_egg(temp) * self.delta_time],
                self.smittu_count[:-1],
            )
        )
